...
FastAPI agent that runs under NSSM on Windows DCS server machines. Replaces the archived node service. Handles DCS process control, log collection, and mission deployment; communicates with the orchestrator over Tailscale.
...
Supported `action` values: `start`, `stop`, `restart`, `logs_bundle`.
...
# Error cases
curl -X POST -H "X-API-Key: change-me" http://localhost:8787/agent/v1/instances/DCS-server1/actions/update
