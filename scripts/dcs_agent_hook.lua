...
local function jstr(s) return '"' .. esc(s) .. '"' end
...
local function refresh_mission()
    ...
    -- Theatre / map name
    local ok2, theatre = pcall(function() return env.mission.theatre end)
    if ok2 and type(theatre) == "string" and theatre ~= "" then
        _status.map = theatre
    end
    -- Fallback: try env.mission.map
    if _status.map == "" then
        local ok3, map = pcall(function() return env.mission.map end)
...
function agentHook.onSimulationFrame()
    if not _mission_loaded then return end
